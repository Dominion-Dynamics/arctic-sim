...
{
  this.name = 'default';
  // PATCHED for arctic-sim: stock gzweb has no fog of any kind. Values mirror
  // the world's <scene><fog>, so the browser view and the camera sensors agree.
  // Override before load with window.GZ3D_FOG = {density: 0.002, color: 0xBFD0DC}.
  var fogCfg = window.GZ3D_FOG || {};
  this.fogDensity = fogCfg.density !== undefined ?
      fogCfg.density : 0.0008 /*ARCTIC_FOG_DEFAULT*/;
  this.fogColor = new THREE.Color(
      fogCfg.color !== undefined ? fogCfg.color : 0xCCD6E0);

  this.scene = new THREE.Scene();
  if (this.fogDensity > 0)
  {
    // Standard materials pick this up automatically; the heightmap uses a
    // custom ShaderMaterial and is fogged explicitly in gzshaders.js.
  ...
  // PATCHED for arctic-sim: stock gzweb clips at 1000 m, which cuts a
  // multi-kilometre terrain off at a hard grey line. Near is raised from
  // 0.1 to 0.5 to keep depth precision sane over the longer range.
  this.camera = new THREE.PerspectiveCamera(60, width / height, 0.5, 50000);
    ...
    segmentWidth, segmentHeight, origin, textures, blends, parent)
{
  ...
  // PATCHED for arctic-sim. Stock gzweb caps the render mesh at 256 segments,
  // which over a multi-kilometre terrain is ~25 m per vertex — far coarser than
  // a screen pixel, so the ground reads as faceted blocks no matter how good
  // the texture is. 1024 renders a 1025^2 heightmap at full resolution
  // (~1M vertices), which modern GPUs handle comfortably.
  // Lower GZ3D_MAX_HEIGHTMAP if you hit a framerate wall.
  var scale = 1;
  var maxHeightmapWidth = window.GZ3D_MAX_HEIGHTMAP || 512;
  var maxHeightmapHeight = window.GZ3D_MAX_HEIGHTMAP || 512;
  ...
  if (textures && textures.length > 0)
  {
    ...
    var options = {
      uniforms:
      {
        ...
        detailAmount: { type: 'f', value: window.GZ3D_DETAIL_AMOUNT !== undefined ? window.GZ3D_DETAIL_AMOUNT : 0.0},
